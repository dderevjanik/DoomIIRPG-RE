...
void renderAutomapContent(Graphics* graphics, Applet* app, const AutomapRenderParams& params) {
	...
	int n3 = params.cellSize > 0 ? params.cellSize : gc.automapCellSize;
	int n6 = 0x400000 / (n3 << 8);
	int n4 = params.originX;
	int n5 = params.originY;
		...
		for (int j = 0; j < 32; ++j) {
			...
			if ((b2 & 0x8) == 0x0) {
				if (app->render->mapEntranceAutomap == (i * 32) + j) {
					...
					graphics->fillRect(n4 + (n3 * j), (n5 - 1) + (n3 * i), n3, n3);
				}
				else if ((b2 & 0x80) != 0x0) {
					if (app->render->mapExitAutomap == (i * 32) + j) {
						...
						graphics->fillRect(n4 + (n3 * j), (n5 - 1) + (n3 * i) + 1, n3, n3);
