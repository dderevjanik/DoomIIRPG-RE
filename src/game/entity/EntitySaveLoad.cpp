...
bool Entity::isBinaryEntity(int* outFlags) {
    ...
    switch (this->def->eType) {
        ...
        case Enums::ET_DOOR: {
            ...
            if (this->def->eSubType == Enums::DOOR_LOCKED && nullptr != outFlags) {
                outFlags[1] |= 0x200000;
            }
            break;
        }
        ...
        case Enums::ET_DECOR_NOCLIP: {
            if (this->def->eSubType == Enums::DECOR_WATER_SPOUT) {
                isBinary = ((this->info & 0x400000) != 0x0);
                break;
            }
            return false;
        }
    ...
    if (nullptr != outFlags) {
        outFlags[0] = (isBinary ? 1 : 0);
    }
    return true;
}

bool Entity::isNamedEntity(int* outName) {
...
void Entity::saveState(OutputStream* OS, int saveFlags) {
    ...
    int sprite = this->getSprite();
    if (this->def->eType == Enums::ET_MONSTER && (saveFlags & 0x200000) != 0x0) {
...
void Entity::loadState(InputStream* IS, int saveFlags) {
    ...
    if ((saveFlags & 0x40000) != 0x0) {
        ...
        if ((this->info & 0x100000) != 0x0 && this->def->eType != Enums::ET_DOOR && (tileFlags < SpriteDefs::getRange("dummy_start") || tileFlags > SpriteDefs::getRange("dummy_end"))) {
    ...
    if ((saveFlags & 0x1000000) != 0x0) {
        this->info |= 0x2000000;
    }
    if (this->isBinaryEntity(nullptr)) {
        this->restoreBinaryState(saveFlags);
        if ((saveFlags & 0x80000) != 0x0) {
            return;
        }
    }
    if (this->def != nullptr && this->def->eType == Enums::ET_ATTACK_INTERACTIVE && this->def->eSubType == Enums::INTERACT_BARRICADE) {
        short scaleFactor = IS->readShort();
        if (scaleFactor != app->render->getSpriteScaleFactor(sprite)) {
            app->render->setSpriteScaleFactor(sprite, scaleFactor);
            this->info |= 0x400000;
        }
        return;
    }
    ...
    if (this->def != nullptr && this->def->eType == Enums::ET_NPC) {
        this->param = (((saveFlags & 0x200000) != 0x0) ? 1 : 0);
        this->param += (((saveFlags & 0x4000000) != 0x0) ? 1 : 0);
    }
    if (this->isMonster()) {
        if ((saveFlags & 0x800000) != 0x0) {
            this->monsterFlags |= Enums::MFLAG_NORESPAWN;
            this->info |= 0x400000;
        }
        if ((saveFlags & 0x400000) != 0x0) {
            this->monsterFlags |= Enums::MFLAG_NOTRACK;
            this->info |= 0x400000;
        }
        if ((saveFlags & 0x8000000) != 0x0) {
    ...
    if (this->def != nullptr && this->def->eType == Enums::ET_MONSTER && (saveFlags & 0x200000) != 0x0) {
        short monsterX = (short)((IS->readByte() & 0xFF) << 3);
        short monsterY = (short)((IS->readByte() & 0xFF) << 3);
        int monsterFrame = (IS->readByte() & 0xFF) << 8;
        app->render->setSpriteX(sprite2, monsterX);
        app->render->setSpriteY(sprite2, monsterY);
        app->render->setSpriteZ(sprite2, (short)(app->render->getHeight(monsterX, monsterY) + 32));
        app->render->setSpriteInfoRaw(sprite2, ((app->render->getSpriteInfoRaw(sprite2) & 0xFFFF00FF) | monsterFrame));
        ...
        if ((saveFlags & 0x40000) == 0x0) {
            ...
            app->game->linkEntity(this, monsterX >> 6, monsterY >> 6);
        }
        this->monsterFlags = IS->readShort();
        if (this->monsterFlags != 0 || monsterFrame != 0) {
            this->info |= 0x400000;
        }
        return;
    }
    ...
    if ((this->info & 0x10000) == 0x0) {
        int infoHi = IS->readByte() & 0xFF;
        int infoMid = IS->readByte() & 0xFF;
        app->render->setSpriteInfoRaw(sprite2, ((app->render->getSpriteInfoRaw(sprite2) & 0xFF0000FF) | infoMid << 8 | infoHi << 16));
        ...
        if (((app->render->getSpriteInfoRaw(sprite2) & 0xF000000) == 0xC000000 || (app->render->getSpriteInfoRaw(sprite2) & 0xF000000) == 0x3000000) && (this->def->eType == Enums::ET_NONOBSTRUCTING_SPRITEWALL || this->def->eType == Enums::ET_SPRITEWALL)) {
            int linkX = this->linkIndex % 32;
            int linkY = this->linkIndex / 32;
            app->render->setSpriteX(sprite2, (short)((linkX << 6) + 32));
            app->render->setSpriteY(sprite2, (short)((linkY << 6) + 32));
            app->render->relinkSprite(sprite2);
        }
        if (infoMid != 0) {
            this->info |= 0x400000;
    ...
    if (this->isMonster()) {
        ...
        uint8_t eType = (uint8_t)(byte1 & 0xF);
        uint8_t eSubType = (uint8_t)(byte1 >> 4 & 0xF);
        uint8_t defParm = IS->readByte();
        this->def = app->entityDefManager->find(eType, eSubType, defParm);
        if (this->name == -1) {
            this->name = (short)(this->def->name | 0x400);
        }
        short tileIdx = this->def->tileIndex;
        if ((saveFlags & 0x100000) != 0x0) {
            tileIdx = this->def->monsterDef->tileIndex;
            int int1 = IS->readInt();
            int int2 = IS->readInt();
            int int3 = IS->readInt();
            if (this->param == 0) {
        ...
        app->render->setSpriteInfoRaw(sprite2, ((app->render->getSpriteInfoRaw(sprite2) & 0xFFFFFF00) | tileIdx));
        app->render->setSpriteEnt(sprite2, this->getIndex());
        if (this->def->eType == Enums::ET_DECOR_NOCLIP && this->def->eSubType == Enums::DECOR_DYNAMITE) {
            int dynamiteFlags = (IS->readByte() & 0xFF) << 24;
            app->render->setSpriteInfoFlag(sprite2, dynamiteFlags);
...
int Entity::getSaveHandle(bool briefSave) {
    ...
    if (droppedEntity && briefSave && this->def->eType == Enums::ET_DECOR_NOCLIP && this->def->eSubType == Enums::DECOR_DYNAMITE) {
        return -1;
    }
    bool binaryFlagSet = tempSaveBuf[0] != 0;
    int saveFlags = tempSaveBuf[1];
    if (binaryEntity && binaryFlagSet) {
        saveFlags |= 0x10000;
    }
    ...
    if ((this->info & 0x400000) == 0x0) {
        saveFlags |= 0x80000;
        if ((saveFlags & 0x20000) == 0x0 && this->def->eType == Enums::ET_DECOR && this->def->eSubType != Enums::DECOR_STATUE) {
            return -1;
        }
    }
    ...
    if ((this->info & 0x10000) != 0x0) {
        saveFlags = ((saveFlags & 0xFFFBFFFF) | 0x100000);
    }
    ...
    if (briefSave) {
        if (this->def->eType == Enums::ET_CORPSE && this->def->eSubType != Enums::CORPSE_SKELETON && !droppedEntity && this->isMonster() && 0x0 == (this->monsterFlags & Enums::MFLAG_NOTRACK)) {
            saveFlags = ((saveFlags & 0xFFFBFFFF) | 0x80000);
        }
...
void Entity::restoreBinaryState(int saveFlags) {

    bool isOpen = (saveFlags & 0x10000) != 0x0;
    switch (this->def->eType) {
        case Enums::ET_ITEM:
        case Enums::ET_ATTACK_INTERACTIVE:
        case Enums::ET_MONSTERBLOCK_ITEM: {
        ...
        case Enums::ET_DOOR: {
            bool wasLocked = this->def->eSubType == Enums::DOOR_LOCKED;
            ...
            if ((saveFlags & 0x200000) != 0x0 || wasLocked) {
                app->game->setLineLocked(this, (saveFlags & 0x200000) != 0x0);
            }
            break;
        }
        case Enums::ET_NONOBSTRUCTING_SPRITEWALL: {
