...
// ─── Visibility pipeline (B3.14b — moved from TinyGL) ──────────────────────

void Render::viewMtxMove(TGLVert* tglVert, int n, int n2, int n3) {
...
TGLVert* Render::transform3DVerts(TGLVert* array, int n) {
...
TGLVert* Render::transform2DVerts(TGLVert* array, int n) {
...
bool Render::clipLine(TGLVert* array) {
...
void Render::projectVerts(TGLVert* array, int n) {
...
bool Render::clippedLineVisCheck(TGLVert* tglVert, TGLVert* tglVert2, bool b) {
...
bool Render::occludeClippedLine(TGLVert* tglVert, TGLVert* tglVert2) {
	int begX = tglVert->x + 7 >> 3;
	int endX = tglVert2->x + 7 >> 3;
	if (begX < 0) {
		begX = 0;
