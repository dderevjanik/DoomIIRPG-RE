...
static void editorLoadMap(int mapID) {
    ...
    camera.posX = (spawnTileX * 64 + 32) / 128.0f;
    camera.posZ = (spawnTileY * 64 + 32) / 128.0f;
    camera.posY = 36.0f / 128.0f;  // eye height
    ...
    std::fprintf(stderr, "Loaded map %d (%d nodes, %d sprites)\n",
                 mapID, app->render->numNodes, app->render->numMapSprites);
}
    ...
    // Load game.yaml
    {
        try {
            ...
            if (YAML::Node game = config["game"]) {
                ...
                if (game["window_title"]) gc.windowTitle = "DoomIIRPG Map Editor";
                else gc.windowTitle = "DoomIIRPG Map Editor";
    ...
    while (running) {
        ...
        if (currentMapID > 0) {
            ...
            // so geometry is visible from outside, but normal culling applies inside
            {
                ...
                bool outside = (viewX < 0 || viewY < 0 ||
                                viewX > mapExtent || viewY > mapExtent);
